        ...
        public async Task InvokeAsync(HttpContext context, RequestDelegate next)
        {
            ...
            if (user != null)
            {
                ...
                if (roleClaims.Any())
                {
                    ...
                    if (menusNames.Any(x => x.Equals(controllerName)) || IsAccessibleForAnyUser(controllerName) || context.Request.Path.Value!.Contains("AccessDenied"))
                    {
                        await next(context);
                    }
                    else
                    {
                        context.Response.Redirect("/Bills/Home/AccessDenied");
                    }
                }
                else
                {
                    var controllerName = context.GetRouteValue("controller")?.ToString();
                    if(IsAccessibleForAnyUser(controllerName) || context.Request.Path.Value!.Contains("AccessDenied"))
                    {
                        await next(context);
                    }
                    else
